...
def run():
    ...
    trust = st.number_input("In %, enter the minimum trustworthiness (e.g. 80)", min_value=0, max_value=100, step=1)
    ...
    if st.button("Calculate Sample size and MDE"):
        if any([baseline_visitors <= 0, baseline_conversions <= 0, risk <= 0, trust <= 0, tails not in ['Greater', 'Two-sided']]):
            ...
            # Adjust visitors per variant based on the number of variants
            visitors_per_variant = np.ceil(baseline_visitors / num_variants)

            # Weekly increments (same number of visitors per week, just distributed across more variants)
            weeks = range(1, 7)  # For 6 weeks
            
            # Prepare a list to store the results for each week
            results = []
            for week in weeks:
                visitors_per_variant_weekly = int(visitors_per_variant * week)
                variant_cr = baseline_rate  # Assuming constant conversion rate over weeks for simplicity

                # Adjust standard error calculation based on visitors per variant
