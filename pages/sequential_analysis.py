...
def get_experiment_data(experiment_id):
    """Fetch all recorded data points for a given experiment ID."""
    try:
        ...
        if len(response.data) > 0:
            ...
            return df
        return pd.DataFrame(columns=['measurement_date', 'visitors', 'conversions'])
    except Exception as e:
        st.error(f"Error fetching data: {e}")
        return pd.DataFrame()

def save_data_point(experiment_id, date, visitors, conversions):
    ...
    * **Rigorous:** Mathematically valid stopping rules, unlike standard z-tests.
    ...
    with st.sidebar:
        ...
        with st.form("setup_form"):
            # If locked, we force the value from DB. If new, we let user type.
            ...
            # The "Start" Button (Only show if NOT locked)
            if not is_locked:
    ...
    with st.form("entry_form"):
        ...
        if st.form_submit_button("Add Data Point"):
            if d_vis < d_conv:
                ...
                save_data_point(exp_id, d_date, d_vis, d_conv)
                st.rerun()
    ...
    if not df.empty:
        ...
        # 1. Use the LOCKED parameters (not mutable ones)
        # Note: In the form above, p0_param etc. hold the correct values because 
        # we initialized them from st.session_state['fetched_params']
