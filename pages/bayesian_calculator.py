...
def display_results(probability_b_better, observed_uplift, probability_winner, aov_a, aov_b, df):
    ...
    if aov_a and aov_b > 0:
        ...
        st.write("")
        st.write("<span style='color: #ff6600;'>*Please enter valid inputs for all fields (business case is optional)</span>",
                 unsafe_allow_html=True)
    ...
    if st.button("Calculate my test results"):
        if valid_inputs:
            ...
            try:
                ...
                display_results(probability_b_better, observed_uplift, probability_winner, aov_a, aov_b, df)
        else:
            st.write("")
            st.write(
